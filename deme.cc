...
Chromosome* Deme::select_parent()
{
  ...
  double random_number = static_cast <double> (rand()) / static_cast <double> (RAND_MAX/sum);
  std::vector<Chromosome*> decending_chromosomes;
  for (int i=0; i<pop_.size(); i++){
    decending_chromosomes.push_back(pop_[i]);
  }
  sort(decending_chromosomes.begin(), decending_chromosomes.end(), compare_fitness);
  double running_sum = 0;
  for (int i=0; i<decending_chromosomes.size(); i++){
    if(decending_chromosomes[i]->get_fitness()+running_sum > random_number){
      return decending_chromosomes[i];
    }
    running_sum += decending_chromosomes[i]->get_fitness();
  }
  return 0;
}
